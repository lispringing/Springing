...
date: '2025-3-9'
updated: '2025-3-9'
description: AI总结:51单片机关于LED灯的一些基本知识，很简单，写的也很通俗易懂
tags:
- 51单片机
categories:
- 学习笔记
...
>就是一开始给单片机的8个小灯全是灭的，也就是说1111 1111 ，然后按一下俺就就开始二进制叠加 1111 1110 这种

但在这要注意的问题是如果一开始给led灯全赋值 1111 1111 然后让led灯的值+1，这样没法加了，因为寄存器只能存8位的，而1111 1111就是封顶了
所以说我们可以给一个变量led赋值 0000 0000 然后使用取反，来操作
```c
#include <REGX52.H>
int i;
int j;
unsigned char LED=0; //这里用char是因为char是8位的，然后寄存器也是8位的
void main(  )//主函数
{
   	while(1)//循环
    {	
